    ...
    <section id="videos" className="relative py-24 md:py-36 bg-card/30">
      <div className="max-w-7xl mx-auto px-5 md:px-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          ...
          {s.youtube_channel_url && s.youtube_channel_url !== "#" && (
            <a
        ...
        {/* Mobile: single column stack. Desktop: masonry-style grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5 md:auto-rows-[200px]">
          {videos.map((v, i) => {
            const thumb =
              v.thumbnail_url ||
              `https://img.youtube.com/vi/${v.youtube_video_id}/maxresdefault.jpg`;

            // Desktop layout spans — mobile ignores these entirely
            const desktopSpan =
            ...
            return (
              <button
                ...
                className={`group relative overflow-hidden rounded-2xl border border-border ${desktopSpan}`}
                style={{
                  // On mobile give each card a fixed aspect ratio since row-span doesn't apply
                  aspectRatio: "16/9",
                }}
              >
                <img
                  ...
                  loading="lazy"
                  onError={(e) => {
                    // Fallback to lower-res YouTube thumb if maxresdefault doesn't exist
                    const t = e.currentTarget;
                    if (t.src.includes("maxresdefault")) {
                      t.src = `https://img.youtube.com/vi/${v.youtube_video_id}/hqdefault.jpg`;
                    }
                  }}
                />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5 text-left">
                  ...
                  <div className="font-display text-lg md:text-2xl font-bold leading-tight">{v.title}</div>
                </div>
              </button>
            );
          })}
